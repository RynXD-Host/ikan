...
IKAN_BANNER = 'Ikan V1 by RynXD <contact@rynxd.link>'
...
class Ikan(object):
    ...
    def fire(self):
        ...
        self.printHeader()
        print("Mau mukul rame² ikan pake mode '{0}' ama {1} orang pake {2} koneksi. Pencet CTRL+C buat batalin.".format(self.method, self.nr_workers, self.nr_sockets))
        ...
        for i in range(int(self.nr_workers)):
            ...
            try:
                ...
                error("Gagal mukul ikan {0}".format(i))
                pass
    ...
    def stats(self):
        ...
        try:
            if self.counter[0] > 0 or self.counter[1] > 0:
                ...
                print("{0} Berhasil mukul ikan. ({1} Gagal)".format(self.counter[0], self.counter[1]))
    ...
    def monitor(self):
        while len(self.workersQueue) > 0:
            try:
                ...
                print("CTRL+C diterima. Bubarin semua orang")
                for worker in self.workersQueue:
                    try:
                        if DEBUG:
                            print("{0} orang bubar".format(worker.name))
                        #worker.terminate()
                        worker.stop()
                    except Exception:
                        pass # silently ignore
...
def usage():
    ...
    print(IKAN_BANNER)
    print()
    print(' USAGE: ./ikan.py <url> [OPTIONS]')
    print()
    print(' OPTIONS:')
...
def main():
    ...
    try:
        ...
        ikan = Ikan(url)
        ikan.useragents = useragents
        ikan.nr_workers = workers
        ikan.method = method
        ikan.nr_sockets = socks
